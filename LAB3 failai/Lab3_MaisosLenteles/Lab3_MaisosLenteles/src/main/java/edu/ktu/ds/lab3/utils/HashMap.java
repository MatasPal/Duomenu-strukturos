...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public java.util.List<V> values() {
        ...
        for(int i = 0; i < table.length; i++)
            if(table[i] != null)
                result.addAll(chainValues(table[i]));
        return result;
    }
    public java.util.List<V> chainValues(Node node) {
        List<V> result = new ArrayList<V>();
        for (Node<K, V> n = node; n != null; n = n.next)
            result.add(n.value);
        return result;
    ...
    public java.util.Set<K> keySet() {
        ...
        for(int i = 0; i < table.length; i++)
            if(table[i] != null)
                result.addAll(chainKeys(table[i]));
        return result;
    }
    public java.util.Set<K> chainKeys(Node node) {
        Set<K> result = new HashSet<>();
        for (Node<K, V> n = node; n != null; n = n.next)
            result.add(n.key);
        return result;
    ...
    public int getNumberOfCollisions(){
        ...
        for(int i = 0; i < table.length; i++)
            for(int j = 1; j < table.length; j++)
                for (Node<K, V> n1 = table[i]; n1 != null; n1 = n1.next)
                   for (Node<K, V> n2 = table[j]; n2 != null; n2 = n2.next)
    ...
    public int averageChainSize(){
        return maxChainSize/chainsCounter;
    }
    ...
    public void replaceAll(V oldValue, V newValue){
        for(int i = 0; i < table.length; i++)
            if(table[i] != null && table[i].value.equals(oldValue)){
            table[i].value = newValue;
        }
    }
    public K putIfAbsent(K key, V value){
        for(int i = 0; i < table.length; i++)
            if(table[i] != null && table[i].key.equals(key) && table[i].value.equals(value)){
                return table[i].key;
            }
        this.put(key, value);
        return null;
    ...
    public java.util.Set<K> keySet(){
        int index = HashManager.hash(hashCode(), table.length, ht);
        re
    }
     */
